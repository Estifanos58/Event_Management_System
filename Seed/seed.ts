...
function buildTempUsersMarkdown(
  users: Array<{ id: string; name: string; email: string; password: string; group: string }>,
) {
  const lines = [
  ...
  users.forEach((user) => {
    lines.push(`| ${user.group} | ${user.name} | ${user.email} | ${user.password} | ${user.id} |`);
  });
...
async function main() {
  ...
  await writeFile("TEMP_USERS.md", buildTempUsersMarkdown(users.profiles), "utf8");
  ...
  const auth = await buildAuthData({
    now: seedNow,
  ...
  const gates = buildGates({
    ...
    staffIds: users.staffIds.length > 0 ? users.staffIds : users.organizerIds,
  });
  ...
  console.log("- Temporary users doc: TEMP_USERS.md");
